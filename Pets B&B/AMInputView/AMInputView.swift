...
@IBDesignable class AMInputView: UIView,UITextFieldDelegate {
    ...
    private func setViewExpandMode(expand:Bool){
        ...
        backViewHeightExpandConstraint.priority = UILayoutPriority(rawValue:(expand ? 500:200))
        labelViewHTopExpandConstraint.priority = UILayoutPriority(rawValue: (expand ? 900:200))
        labelViewRightExpandConstraint.priority = UILayoutPriority(rawValue:(expand ? 900:300))
        labelViewLeftExpandConstraint.priority = UILayoutPriority(rawValue: (expand ? 999:300))
        ...
        UIView.animate(withDuration: animationDuration) {
            ...
            if expand {
                ...
                let leftMargin = 0
//                    ( -1 * ((1-scale)*(self.labelView.layer.bounds.size.width))/2 - 11))
                self.labelView.transform = self.labelView.transform.translatedBy(x: CGFloat(leftMargin), y: 0)
            }
        }
    }
